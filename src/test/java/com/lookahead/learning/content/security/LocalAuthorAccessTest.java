...
class LocalAuthorAccessTest {
    ...
    private AccountPrincipal author() {
        return new AccountPrincipal(LocalAuthorAccess.ACCOUNT_ID, LocalAuthorAccess.USERNAME, "Author", null, true);
    }
    @Test void onlyTheReservedLocalAccountHasTheCapability() {
        ...
        assertThat(access.allowed(new AccountPrincipal(UUID.randomUUID(), LocalAuthorAccess.USERNAME, "Author", null, true))).isFalse();
        assertThat(access.allowed(new AccountPrincipal(LocalAuthorAccess.ACCOUNT_ID, "learner01", "Author", null, true))).isFalse();
        assertThat(access.allowed(new AccountPrincipal(LocalAuthorAccess.ACCOUNT_ID, LocalAuthorAccess.USERNAME, "Author", null, false))).isFalse();
