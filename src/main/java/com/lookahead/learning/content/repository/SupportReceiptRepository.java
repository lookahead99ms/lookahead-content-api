...
public class SupportReceiptRepository implements SupportReceiptStore {
    ...
    @Override public Reservation reserve(String owner, String key, String digest) {
        return transaction.execute(status -> {
            ...
            // Account lock serializes request-key creation and the per-account rate limit across replicas.
            var users=jdbc.queryForList("SELECT id FROM accounts WHERE id=? AND enabled=true FOR UPDATE",UUID.class,account);
